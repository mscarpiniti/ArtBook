...
/* Whisper CallBack function con STFT ------------------- */
static int whisper_Callback( const void* inputBuffer, void* outputBuffer,
                      ...
                      void* userData )
{
    ...
    // Impongo la fase come numero casuale in [0, 2\pi]
    for (i = 0; i < N; i++) {
        m = sqrt(out[i][0] * out[i][0] + out[i][1] * out[i][1])/N;
        phi = ((double)rand() / (double)RAND_MAX) * 2 * (double)M_PI;
        out[i][0] = m * cos(phi);
        out[i][1] = m * sin(phi);
    }
