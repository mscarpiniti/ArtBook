...
/* Denoising CallBack function con STFT ----------------- */
static int denoising_Callback( const void* inputBuffer, void* outputBuffer,
                        ...
                        void* userData )
{
    ...
    // Applico il denoising
    for (i = 0; i < N; i++) {
        m = sqrt(out[i][0] * out[i][0] + out[i][1] * out[i][1]);
        out[i][0] = (m * out[i][0] / (m + (double)DELTA))/N;
        out[i][1] = (m * out[i][1] / (m + (double)DELTA))/N;
    }
