...
/* OVERLAP AND ADD CallBack function ------------------- */
static int OLA_Callback( const void* inputBuffer, void* outputBuffer,
                      ...
                      void* userData )
{
    ...
    // Inserisco i nuovi campioni nel segnale di input
    ...
    // Overlap and Add
    for (i = 0; i < L; i++) {
        buffer[pt_s] = buffer[pt_s] + outt[i][0];
        pt_s++;
        if (pt_s >= LB)
            pt_s = 0;
    }
